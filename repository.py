...
class Repository:
    ...
    def create_tables(self):
        vaccines_data, suppliers_data, clinics_data, logistics_data = self.init_config()
        cursor = self.conn.cursor()
        cursor.execute(
            'CREATE TABLE Logistics(id INTEGER PRIMARY KEY,name STRING NOT NULL, count_sent INTEGER NOT NULL, count_received INTEGER NOT NULL)')
        self.insert_logistics(logistics_data)
        cursor.execute(
            'CREATE TABLE Clinics(id INTEGER PRIMARY KEY, location STRING NOT NULL, demand INTEGER NOT NULL, FOREIGN KEY(logistic) REFERENCES Logistic(id))')
        self.insert_clinics(clinics_data)
        cursor.execute(
            'CREATE TABLE Suppliers(id INTEGER PRIMARY KEY, name STRING NOT NULL, FOREIGN KEY(logistic) REFERENCES Logistic(id))')
        self.insert_suppliers(suppliers_data)
        cursor.execute(
            'CREATE TABLE Vaccines(id INTEGER PRIMARY KEY, date DATE NOT NULL, FOREIGN KEY(supplier) REFERENCES Supplier(id), quantity INTEGER NOT NULL)')
